...
class PIIDetector(Validator):
    ...
    PII_PATTERNS = {
        ...
        "PHONE":       r"(?<!\d)(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]\d{3}[-.\s]\d{4}\b",
    ...
    def validate(self, value: str, metadata: dict):
        """
        Tìm PII trong value; nếu phát hiện, trả về FailResult với fix_value đã redact.
        ...
        Bước:
          ...
          3. Nếu found_pii không rỗng → FailResult(fix_value=redacted_text) — để
             on_fail=OnFailAction.FIX thay thế đầu ra bằng bản đã redact.
        ...
        if found_pii:
            ...
            return FailResult(
                error_message=f"Phat hien PII: {', '.join(sorted({p[0] for p in found_pii}))}",
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        ...
        Trả về PassResult nếu JSON đã hợp lệ ngay từ đầu (không cần sửa).
        Trả về FailResult(fix_value=...) nếu cần sửa hoặc không thể sửa được —
        on_fail=OnFailAction.FIX chỉ thay thế validated_output khi validate()
        trả về FailResult; PassResult(value_override=...) không được Guard áp
        dụng vào validated_output trong guardrails-ai 0.11.x (đã verify bằng test).
        """
        try:
            json.loads(value)
            return PassResult()
        ...
        try:
            ...
            return FailResult(
                error_message="JSON cần được sửa lỗi định dạng (fences/nháy đơn/phẩy thừa)",
...
def main(demo: str = "all"):
    """
    demo: "pii" | "json" | "all" — cho phép chạy riêng từng demo để lưu
    evidence log tách biệt (04_pii_demo_log.txt / 04_json_demo_log.txt)
    mà không phá vỡ run_all.py (vốn gọi main() trực tiếp, không qua CLI).
    """
    print("=" * 55)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--demo", choices=["pii", "json", "all"], default="all",
        help="Chạy demo PII, demo JSON, hoặc cả hai (mặc định: all)",
    )
