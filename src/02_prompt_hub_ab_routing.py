...
PROMPT_V1_NAME = "nguyenviethung-rag-prompt-v1"
PROMPT_V2_NAME = "nguyenviethung-rag-prompt-v2"
...
SYSTEM_V1 = (
    ...
    "Nếu context không chứa thông tin cần thiết, hãy nói thẳng là không biết, không suy đoán thêm.\n\n"
    "Context:\n{context}"
)
...
SYSTEM_V2 = (
    ...
    "sau đó trả lời theo cấu trúc: "
    "1) Tóm tắt câu trả lời chính, "
    "2) Trích dẫn nguồn từ context, "
    "3) Nêu rõ mức độ chắc chắn của câu trả lời (3-5 câu tổng cộng). "
...
def pull_prompts_from_hub(client: Client) -> dict:
    ...
    # client.pull_prompt() đăng ký lại filter riêng khi deserialize, ghi đè
    # warnings.filterwarnings("ignore") toàn cục ở đầu file → cần catch_warnings
    # cục bộ để chặn LangChainPendingDeprecationWarning một cách đáng tin cậy.
